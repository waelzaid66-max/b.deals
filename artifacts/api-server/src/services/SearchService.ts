...
/**
 * Near-me / radius filter shared by list search and map clustering. Requires a
 * LEFT JOIN on `locations` (1:1 via location_id). Additive: returns [] when geo
 * params are absent so existing behaviour is unchanged.
 */
export function nearMeConditions(parsed: ParsedSearchQuery): SQL[] {
  if (
